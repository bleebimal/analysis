...
def ipc_read():
    ...
    dataframe_ipc_recall = pd.DataFrame(tuple_ipc_recall, columns=['ipc', 'recall']).sort_values('recall', ascending=False).head(30)  # ipc recall to data frame
    ...
    try:
        ...
        print(ipc_precision)
        ipc_precision_final = pd.DataFrame.from_dict(ipc_precision, orient='index', columns=['IPC_Precision'], dtype='float64').sort_values('IPC_Precision', ascending=False).head(10)
        ...
        dataframe_ipc_precision = pd.DataFrame(tuple_ipc_precision, columns=['ipc', 'precision']).sort_values('precision',ascending=False).head(30)
        ...
        for rindex, rvalue in dataframe_ipc_recall.iterrows():
            for pindex, pvalue in dataframe_ipc_precision.iterrows():
                if rvalue['ipc'] == pvalue['ipc']:
                    ipc_mpr.update({rvalue['ipc']: ('%.6f' % ((float(rvalue['recall']) + float(pvalue['precision']))/2))})
        ...
        ipc_mpr_show = pd.DataFrame(tuple_ipc_mpr, columns=['ipc', 'mpr']).sort_values('mpr', ascending=False).head(5)

        ipc_mpr_final = pd.DataFrame.from_dict(ipc_mpr, orient='index', columns=['IPC_MPR'], dtype='float64').sort_values('IPC_MPR', ascending=False).head(10)
...
def cpc_read():
    ...
    dataframe_cpc_recall = pd.DataFrame(tuple_cpc_recall,  columns=['cpc', 'recall']).sort_values('recall',ascending=False).head(30)  # cpc recall to data frame
    ...
    try:
        ...
        cpc_precision_final = pd.DataFrame.from_dict(cpc_precision, orient='index', columns=['CPC_Precision'], dtype='float64').sort_values('CPC_Precision', ascending=False).head(10)
        ...
        dataframe_cpc_precision = pd.DataFrame(tuple_cpc_precision, columns=['cpc', 'precision']).sort_values('precision',ascending=False).head(30)
        ...
        for rindex, rvalue in dataframe_cpc_recall.iterrows():
            for pindex, pvalue in dataframe_cpc_precision.iterrows():
                if rvalue['cpc'] == pvalue['cpc']:
                    cpc_mpr.update({rvalue['cpc']: ('%.6f' % ((float(rvalue['recall']) + float(pvalue['precision']))/2))})
        ...
        cpc_mpr_show = pd.DataFrame(tuple_cpc_mpr, columns=['cpc', 'mpr']).sort_values('cpc', ascending=False).head(5)

        cpc_mpr_final = pd.DataFrame.from_dict(cpc_mpr, orient='index', columns=['CPC_MPR'], dtype='float64').sort_values('CPC_MPR', ascending=False).head(10)
...
def upc_read():
    ...
    try:
        ...
        upc_precision_final = pd.DataFrame.from_dict(upc_precision, orient='index', columns=['UPC_Precision'], dtype='float64').sort_values('UPC_Precision', ascending=False).head(10)
        ...
        dataframe_upc_precision = pd.DataFrame(tuple_upc_precision, columns=['upc', 'precision']).sort_values('precision',ascending=False).head(30)
        ...
        for rindex, rvalue in dataframe_upc_recall.iterrows():
            for pindex, pvalue in dataframe_upc_precision.iterrows():
                if rvalue['upc'] == pvalue['upc']:
                    upc_mpr.update({rvalue['upc']: ('%.6f' % ((float(rvalue['recall']) + float(pvalue['precision']))/2))})
        ...
        upc_mpr_show = pd.DataFrame(tuple_upc_mpr, columns=['upc', 'mpr']).sort_values('upc',
                                                                                             ascending=False).head(5)

        upc_mpr_final = pd.DataFrame.from_dict(upc_mpr, orient='index', columns=['UPC_MPR'], dtype='float64').sort_values('UPC_MPR', ascending=False).head(10)
